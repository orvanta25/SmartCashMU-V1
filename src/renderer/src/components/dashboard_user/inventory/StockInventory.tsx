...
import { User, Barcode, Package, Hash, Save, Loader2, Plus } from "lucide-react";
import { Link } from "react-router-dom";
import { useAuth } from "../../auth/auth-context";
import { toast } from "react-toastify";
import { getProductByBarcode } from "@renderer/api/produit";
import { createInventory } from "@renderer/api/inventaire";
...
export function StockInventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await createInventory(entreprise.id,{
      ...
      setFormData({ responsable: "", codeBar: "", designation: "", quantity: undefined as any });
    ...
    <div className="min-h-screen bg-orvanta py-4 px-4">
      <div className="max-w-3xl mx-auto space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Link
            to="/dashboard_user/inventory/stock/list"
        ...
        <div className="bg-gradient-to-b from-white/5 to-white/[0.02] backdrop-blur-xl rounded-md border border-white/10 shadow-lg">
          <div className="px-4 py-3 border-b border-white/10">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-base font-semibold text-white">Ajouter un article</h2>
