...
import { createAchatFournisseur, updateAchatFournisseur } from "@renderer/api/achat-fournisseur"
import type { CreateAchatFournisseurDto, UpdateAchatFournisseurDto, CreateEntreeDto } from "@renderer/types/achat-entree"
...
const InvoiceForm = () => {
  ...
  const [achatFournisseurId, setAchatFournisseurId] = useState<string | null>(null)
 ...
 const handleProductChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // Définir quels champs sont numériques
  const isNumericField = ['quantity', 'puht', 'tva', 'remiseArticle'].includes(name)
  const isTextField = ['codeBar', 'designation'].includes(name)
  
  let formattedValue = value
  
  // Remplacer les virgules par des points (pour tous les champs)
  if (formattedValue.includes(',')) {
    formattedValue = formattedValue.replace(',', '.')
  }
  
  // Validation différente selon le type de champ
  if (isNumericField) {
    // Pour les champs numériques : accepter seulement chiffres, points, signe moins
    if (formattedValue && !/^-?\d*\.?\d*$/.test(formattedValue)) {
      return // Rejette les lettres
    }
  } 
  else if (isTextField) {
    // Pour les champs texte : accepter tout (lettres, chiffres, symboles)
 ...
 const handleSubmit = async (e: FormEvent) => {
  ...
  try {
    // 1. Construire le tableau d'entrées à partir des produits
    ...
    // 2. Construire le DTO selon l'interface du backend
    const dto: CreateAchatFournisseurDto = {
    ...
    console.log("Envoi du DTO au backend:", JSON.stringify(dto, null, 2))

    // 3. UN SEUL appel API avec toutes les entrées
    const response = await createAchatFournisseur(entreprise.id, dto, form.pieceJointe)
    
    console.log("Réponse du backend:", response)

    if (!response.achatFournisseur?.id) {
      throw new Error("La création de la facture a échoué")
    }

    // 4. Mettre à jour l'état avec la réponse du backend
    setForm(prev => ({
      ...prev,
      montantTotal: response.achatFournisseur.montantTotal || 0,
      montantRestant: response.achatFournisseur.montantRestant || 0,
      montantHT: response.achatFournisseur.entrees.reduce((sum, e) => sum + (e.puht * e.quantite), 0),
      montantTVA: response.achatFournisseur.entrees.reduce((sum, e) => sum + (e.puht * e.quantite * e.tva / 100), 0)
    }))

    setAchatFournisseurId(response.achatFournisseur.id)
    
    // Mettre à jour les produits avec les données retournées par le backend
    setProducts(prev => prev.map((product, index) => {
      const backendProduct = response.achatFournisseur.entrees[index]
      if (!backendProduct) return product
      
      return {
        ...product,
        prixUnitaireTTC: backendProduct.prixUnitaireTTC,
        prixTotalTTC: backendProduct.prixTotalTTC,
        prixTotalHT: backendProduct.puht * backendProduct.quantite
      }
    }))

    toast.success(`Facture ${form.numeroFacture} créée avec succès - ${products.length} article(s)`)
    
    // Optionnel : réinitialiser pour une nouvelle facture
    // handleNewInvoice()
  ...
  const handleNewInvoice = () => {
    ...
    setAchatFournisseurId(null)
    setEditingProductIndex(null)
    toast.success("Nouvelle facture prête")
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-white/60">Chargement de l'application...</p>
        </div>
      </div>
    )
  }
            ...
            <div>
              <h1 className={`text-3xl font-bold text-white ${responsive.heading}`}>
                <span className="bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent">
                  <Receipt className="inline-block mr-3 w-8 h-8" />
                  Nouvelle Facture
                </span>
              </h1>
              <p className="text-white/60 mt-2">Créez et gérez vos factures fournisseurs</p>
            </div>
                      ...
                      <div className="relative group">
                        ...
                        <div className="flex items-center justify-center gap-2 px-4 py-3 bg-gray-800/50 border-2 border-dashed border-gray-700 rounded-xl group-hover:border-purple-500/50 transition-colors duration-200">
                          ...
                          <span className="text-gray-400">
                            {form.pieceJointe ? form.pieceJointe.name : 'Cliquez pour ajouter un fichier'}
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <table className="w-full min-w-full">
                        ...
                        <tbody className="divide-y divide-gray-800">
                          {products.map((product, index) => (
                            <tr key={product.id} className="hover:bg-gray-800/30 transition-colors duration-150">
